...
const formSchema = z.object({
  namaSubPaket: z
    .string()
    .min(3, { message: "Nama sub paket harus diisi (min. 3 karakter)." }),
  idPaket: z.string().min(1, { message: "Paket harus dipilih." }),
  hargaIDR: z.coerce
    .number()
    .gt(0, { message: "Harga IDR harus lebih dari 0." }),
  hargaUSD: z.coerce
    .number()
  ...
  durasiHari: z.coerce
    .number()
    .min(1, { message: "Durasi harus diisi (min. 1 hari)." }),
  penerbangan: z.string().optional(),
  hotelMekkah: z
    .string()
    .min(3, { message: "Informasi hotel Mekkah harus diisi." }),
  hotelMadinah: z
    .string()
    .min(3, { message: "Informasi hotel Madinah harus diisi." }),
...
function EditSubPaketPage() {
  ...
  const {
    ...
    append: appendFasilitas,
    remove: removeFasilitas,
  } = useFieldArray({
  ...
  const {
    fields: perlengkapanFields,
    append: appendPerlengkapan,
    remove: removePerlengkapan,
  } = useFieldArray({
    control: form.control,
    name: "perlengkapan",
  });
        ...
        .then((data) => {
          ...
          form.reset({
            ...
            fasilitas: p.fasilitas
              ? p.fasilitas.split(",").map((f) => ({ value: f.trim() }))
              : [{ value: "" }],
            perlengkapan: p.perlengkapan
              ? p.perlengkapan.split(",").map((p) => ({ value: p.trim() }))
              : [{ value: "" }],
          });
          ...
          setHargaUSDDisplay(
            p.hargaUSD ? formatCurrency(p.hargaUSD, "en-US", "USD") : null,
          );
          setCurrentImage(
            p.urlFoto
              ? `${process.env.NEXT_PUBLIC_API_BASE_URL}${p.urlFoto}`
              : null,
          );
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    const updatedSubPaket = {
      ...
      keberangkatan: values.keberangkatan
        ? new Date(values.keberangkatan).toISOString()
        : "", // Biarkan undefined jika tidak diisi
      ...
      fasilitas: values.fasilitas
        ...
        .filter((f) => f)
        .join(","),
      perlengkapan: values.perlengkapan
        .map((p) => p.value.trim())
        .filter((p) => p)
        .join(","),
    };
            ...
            >
                      ...
                      <FormControl>
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            setHargaIDRDisplay(
                              formatCurrency(numeric, "id-ID", "IDR"),
                            );
                            form.setValue("hargaIDR", numeric, {
                              shouldValidate: true,
                            });
                          }}
                        />
                ...
                <FormField
                  ...
                  render={() => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          value={hargaUSDDisplay ? hargaUSDDisplay : ""}
                          onChange={(e) => {
                            const numeric = unformatCurrency(e.target.value);
                            setHargaUSDDisplay(
                              formatCurrency(numeric, "en-US", "USD"),
                            );
                            form.setValue("hargaUSD", numeric, {
                              shouldValidate: true,
                            });
                          }}
                        />
              ...
              <FormItem>
                      ...
                      <FormItem>
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            disabled={fasilitasFields.length <= 1}
                          >
                ...
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => appendFasilitas({ value: "" })}
                >
                  <Plus className="w-4 h-4 mr-1" /> Tambah Fasilitas
                </Button>
                <FormMessage>
                  {form.formState.errors.fasilitas?.message}
                </FormMessage>
              </FormItem>
              ...
              <FormItem>
                      ...
                      <FormItem>
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            disabled={perlengkapanFields.length <= 1}
                          >
                            <Minus className="w-4 h-4" />
                          </Button>
                        </div>
                ...
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => appendPerlengkapan({ value: "" })}
                >
                  <Plus className="w-4 h-4 mr-1" /> Tambah Perlengkapan
                </Button>
                <FormMessage>
                  {form.formState.errors.perlengkapan?.message}
                </FormMessage>
              </FormItem>
