...
function MasterSubPaket() {
    ...
    if (confirmed.confirmed) {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/master/sub-paket/${subPaket.idSubpaket}`,
          {
            method: "DELETE",
            headers: {
        ...
        if (!response.ok) {
          toast.error(
            "Gagal menghapus sub paket. Pastikan tidak ada data terkait yang masih digunakan.",
          );
        }
  ...
  const headers: Header<SubPaket>[] = [
    ...
    { column: "paket.nama", label: "Paket", orderable: true, align: "left" },
    {
      ...
      render: (row: SubPaket) => (
        <img
          src={baseUrl + row.urlFoto || "/images/no-image.png"}
          alt={row.namaSubPaket}
    ...
    },
    {
      ...
      label: "IDR",
      align: "right",
      render: (row: SubPaket) => formatCurrency(row.hargaIDR, "id-ID", "IDR"),
    },
    {
      column: "hargaUSD",
      label: "USD",
    ...
    },
    {
      ...
      orderable: true,
      align: "right",
      render: (row: SubPaket) => `${row.durasiHari} Hari`,
    },
      ...
      render: (row: SubPaket) => (
        <>
          <div className="flex gap-2">
            <Link
              href={`/master-subpaket/view/${encrypt(row.idSubpaket)}`}
              passHref
            >
              <Button
                variant="outline"
            ...
            <Link
              href={`/master-subpaket/edit/${encrypt(row.idSubpaket)}`}
              passHref
            >
              <Button
                variant="outline"
          ...
          <nav aria-label="Breadcrumb">
            <ol className="flex items-center gap-1 text-sm text-gray-700 font-bold">
              <li>
                <a
                  href="#"
                  className="block transition-colors hover:text-gray-900 text-base"
                >
                  {" "}
                  Admin{" "}
                </a>
